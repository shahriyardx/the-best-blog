...
interface Props {
  ...
  comments: Pick<Comment & { author: Pick<User, "username"> }, "author_id" | "post_id" | "content">[]
  refetch: () => void
}
...
const LikesAndComments = ({ likes, comments, refetch, post_id}: Props) => {
  ...
  const liked = () => {
    return likes.find(like => like?.user_id === session?.profile?.id) ? true : false
  }
  
  const LikeIcon = liking ? AiOutlineLoading3Quarters : liked() ? AiFillHeart : AiOutlineHeart
  ...
  return (
    <div className='py-5 xl:py-0 flex items-center gap-5 dark:border-b-zinc-800 dark:border-t-zinc-800 border-b-2 border-t-2 mb-5 xl:border-b-0 xl:border-t-0'>
      <button onClick={addLike} className={`flex items-center gap-2 text-lg rounded-md ${liking && 'cursor-wait'}`}>
        <LikeIcon className={`${liked() && !liking && 'text-red-500'} ${liking && 'animate-spin'} text-2xl`} /> 
